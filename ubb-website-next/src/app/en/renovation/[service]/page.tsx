...
export default async function EnRenovationSubPage({ params }: { params: Promise<{ service: string }> }) {
  const { service } = await params
  const sub = cat.subServices.find((s) => s.slug === service)
  if (!sub) notFound()
  ...
  return (
    <>
      <section className="hero hero-slim" style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="wrap" style={{ position: 'relative', zIndex: 2 }}>
          <div className="breadcrumb">
            ...
            <Link href="/en/renovation">Renovation</Link><span className="sep">/</span>
            {sub.title}
          </div>
          <span className="kicker">{sub.num} — Renovation &amp; Conversion</span>
          <h1>{sub.title}</h1>
      ...
      {relProjects.length > 0 && (
        <section className="section">
          <div className="wrap">
            <div className="section-head section-head-v"><span className="kicker">References</span><div><h2>Projects from the region.</h2></div></div>
