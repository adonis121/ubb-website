...
export default async function EnFlooringSubPage({ params }: { params: Promise<{ service: string }> }) {
  const { service } = await params
  const sub = cat.subServices.find((s) => s.slug === service)
  if (!sub) notFound()
      ...
      <section className="hero hero-slim" style={{ position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="wrap" style={{ position: 'relative', zIndex: 2 }}>
          <div className="breadcrumb">
            ...
            <Link href="/en/flooring">Floor Works</Link><span className="sep">/</span>
            {sub.title}
          </div>
          <span className="kicker">{sub.num} — Floor Works</span>
          <h1>{sub.title}</h1>
